	...
	<head>
		...
		<script type="text/javascript">
			function importAdminEnforceExcel(){
				if(confirm("操作将导入行政执法检查情况统计表,是否确认?")){
...
var fm = new Ext.FormPanel({
    title : '批量导入行政执法检查情况统计表文件',
    url : 'upload/trfmlisn.svt?type=aeinspectionanl&t=' + new Date(),
        ...
        handler : function() {
            ...
            if(fm.getForm().isValid() && filePath != null && filePath != '') {//验证form
                if(filePath.substring(filePath.lastIndexOf(".") + 1).toLowerCase() != "xls") {
                    ...
                    Ext.Msg.confirm('提示', '是否确认导入行政执法检查情况统计表？', function(btn) {
                        if(btn == 'yes') {
                            //显示进度条
                            Ext.MessageBox.show({
                                title : '正在上传文件',
                            ...
                            var timer = setInterval(function() {
                                ...
                                Ext.Ajax.request({
                                    ...
                                    //否则极有可能每次出现的数据都是一样的，
                                    //这和浏览器缓存有关
                                    url : 'upload/prcscontl.svt?type=aeinspectionanl&t=' + new Date(),
                                    method : 'get',
                                    //处理ajax的返回数据
                                    success : function(response, options) {
                                        ...
                                        if(obj.success != false) {
                                            if(obj.finished) {
                                                ...
                                                Ext.MessageBox.hide();
                                                Ext.Msg.alert('提示', '行政执法检查情况统计表Excel处理完成！');
                                                Ext.getCmp('logMessage').setValue('行政执法检查情况统计表Excel处理完成！');
                                            } else {
                                    ...
                                    failure : function() {
                                        clearInterval(timer);
                                        Ext.Msg.alert('错误', '批量导入行政执法检查情况统计表文件发生错误');
                                    }
                                });
                            }, 500);
                        }
