...
public class BaseAction extends Action {
    ...
    public List<BsDictionary> getDicMap(String type) {
        List<BsDictionary> list = bsDictionaryDao.find(
                "from BsDictionary where type = ? and isenable = 'true' order by id",
                new Object[] { type });
        return list;
    }
    ...
    public Map<String, String> getDicMap() {
        List<BsDictionary> list = bsDictionaryDao.find(
                "from BsDictionary where isenable = 'true' order by createDate", new Object[] {});
    ...
    protected Boolean isPcbUser(HttpServletRequest request, HttpServletResponse response) {
        String ispcbuser = this.getPubCredential(UserCredentialName.ispcbuser.name(), request,
                response);
        if (ispcbuser == null) {
            return false;
        }
     ...
     * @param url
     */
    public void printMessageOnlyJs(HttpServletRequest request, HttpServletResponse response,
     ...
     * @param url
     */
    public void printMessage(HttpServletRequest request, HttpServletResponse response, String msg,
            String url) {
        try {
            response.setContentType("text/html");
            response.setCharacterEncoding("UTF-8");
            ...
            StringBuffer buf = new StringBuffer();
            buf.append("<html><head><link href=\"").append(request.getContextPath())
                    .append("/css/css.css\"").append(" rel=\"stylesheet\" type=\"text/css\" />");
            ...
            buf.append(
                    ...
                    .append("<tr>")
                    .append("<td height=\"35\" class=\"nawzjj\">")
                    .append("<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">")
                    .append("<tr>").append("<td class=\"ndwz\">").append("&nbsp;").append("</td>")
                    .append("<td class=\"ndwzzml\">").append(projectName).append("<img src=\"")
                    .append(request.getContextPath()).append("/images/index11.jpg\" ")
                    .append(" width=\"6\" height=\"10\" hspace=\"5\" />").append("系统错误提示")
                    .append("</td>").append("</tr>").append("</table>").append("</td>")
    ...
    public void printMessageTemp(HttpServletRequest request, HttpServletResponse response,
            String msg, String url) {
        try {
            response.setContentType("text/html");
            response.setCharacterEncoding("UTF-8");
    ...
    public void printMessageRe(HttpServletRequest request, HttpServletResponse response, String url)
            throws IOException {
        response.setContentType("text/html");
        response.setCharacterEncoding("UTF-8");
        PrintWriter out = response.getWriter();
    ...
    public void printMessageToParent(HttpServletRequest request, HttpServletResponse response,
    ...
    public String getPubCredential(String name, HttpServletRequest request,
            HttpServletResponse response) {
        Subject subject = this.getSubject(request, response);
        String value = "";
        try {
    ...
    public boolean hasPrincipals(String principalsStr, String operator, HttpServletRequest request,
        ...
        for (int j = 0; j < principalsArray.size(); j++) {
            ...
            ppal = PrincipalUtils.getPrincipal(RolePrincipal.class, new Class[] { String.class,
     ...
     * @return
     */
    public boolean hasPermission(String actions, HttpServletRequest request,
            HttpServletResponse response) {
        Subject subject = this.getSubject(request, response);
        if (subject == null) {
            return false;
        ...
        try {
            permission = (Permission) PermissionUtils.getPermission(URLPermission.class.getName(),
                    "dumy name", actions);
    ...
    public boolean authorized(String uri, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            urlPermission = (Permission) PermissionUtils.getPermission(
                    URLPermission.class.getName(), "dummy name", actions.toString());
        } catch (ClassNotFoundException e) {
            return false;
        }
    ...
    protected String getDownloadFileName(final String fileName, final String attType) {
        ...
        String attchName = fileName.replace("（", "").replace("）", "").replace("【", "")
                .replace("】", "");
        LOGGER.debug("下载文件名（替换符号后）：fileName=" + attchName + ",size is :" + attchName.length());
        try {
            attchName = URLEncoder.encode(
                    String.format("%s_%s.doc", StringUtils.trimToEmpty(attchName),
                            StringUtils.trimToEmpty(attType)), CharEncoding.UTF_8);
