...
public class AdministrationEnforceManagerAction extends BaseAction {
	 ...
	 * 跳转行政执法信息Excel导入页面.
	 */
	public ActionForward toAdminEnforceExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
	 ...
	 */
	public ActionForward toAEInspectionAnlExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
	 ...
	 * @author yangyz
	 */
	public ActionForward toAeinspectionAnl(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
	...
	public ActionForward generateAeinspectionAnlExcel(ActionMapping mapping, ActionForm form,
		...
		String nowLoginUser = this.getPubCredential(UserCredentialName.organization.name(), request, response);
		...
		try {
			formA1 = java.net.URLDecoder.decode(request.getParameter("form_a1"), CharEncoding.UTF_8);
			formB1 = java.net.URLDecoder.decode(request.getParameter("form_b1"), CharEncoding.UTF_8);
		...
		try {
			...
			String fileName = String.format("行政执法检查情况统计表-%1$tm%1$td%1$tH%1$tM%1$tS.xls", new Date());
	 ...
	 * 行政执法信息Excel导入保存方法.
	 */
	public ActionForward importAdminEnforceExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
	 ...
	 * 行政执法登记保存方法.
	 */
	public ActionForward createAdminEnforce(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			...
			try {
				...
				ae.setStep("1");
				ae.setAeno(noGenerator.generateNoStr(dyna.get("aetext").toString(), dyna.get("aeyear").toString(), dyna
				        .get("aeindex").toString()));
				FormFile file = null;
				...
				ae.setAebasis(StringUtil.join(aebasisArr, StringUtils.EMPTY, StringUtils.EMPTY, ","));
				ae.setAeway(StringUtil.join(aewayArr, StringUtils.EMPTY, StringUtils.EMPTY, ","));
				// 保存创建人为当前登录人id
				ae.setCrtlogin(this.getPrivCredential(UserCredentialName.login.name(), request, response));
				...
				try {
					...
					request.setAttribute(ATTR_MESSAGE,
					        "<b>保存行政执法登记操作失败,编号重复</b><br><b>失败原因:</b>因为在你保存执法信息之前有其他用户已经保存了执法编号 " + ae.getAeno()
					                + " 的执法信息<br><b>解决方法:</b>请重新点击'行政执法登记'菜单获取新执法编号，再输入保存!");
					return mapping.findForward("toAdminEnforceMessage");
				}
				request.setAttribute(ATTR_MESSAGE, "保存行政执法登记【" + ae.getAeno() + "】操作成功!");
				LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "保存行政执法登记", "操作成功!"));
			} catch (Exception e) {
	 ...
	 * @return
	 */
	public ActionForward getAepeoplesByPcbOrgNo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		...
		StringBuffer outputSb = new StringBuffer("{totalProperty:" + childrenAepeopleslt.size() + ",root:[");
		BsAepeople aepeople = null;
		for (int i = 0; i < childrenAepeopleslt.size(); i++) {
			aepeople = childrenAepeopleslt.get(i);
			outputSb.append("{'value':'").append(aepeople.getPepname()).append("--").append(aepeople.getCreditno())
			        .append("','text':'").append(aepeople.getPepname()).append("-").append(aepeople.getCreditno())
			        .append("'}");
	 ...
	 * 跳转行政执法信息列表页面.
	 */
	public ActionForward toAdminEnforceList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		String aeno = (dyna.get("aeno") == null || dyna.get("aeno").equals(StringUtils.EMPTY)) ? null : (String) dyna
		        .get("aeno");
		...
		if (source != null && source.equals("2")) {
			page = enforceService.queryAdmenforceForInspc(aeno, aeorgChoice, aeedorgChoice, aeplanstdate, aeplantmdate,
			        Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		} else {
			page = enforceService.queryAdmenforce(aeno, aeorgChoice, aeedorgChoice, aeplanstdate, aeplantmdate,
			        Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		}
		...
		for (Iterator<?> iterator = list.iterator(); iterator.hasNext();) {
			...
			if (object != null && object instanceof BsAdmenforce) {
				...
				waitViewAdapt.setAeedorgnm(StringUtils.abbreviate(waitViewAdapt.getAeedorgnm(), 15));
			}
		}
	 ...
	 * 跳转行政执法信息详细页面.
	 */
	public ActionForward toAdminEnforceDetailPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
	 ...
	 * 跳转行政执法信息修改页面.
	 */
	public ActionForward toUpdateAdminEnforcePage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		this.saveToken(request);// 产生令牌值
		...
		if (CollectionUtils.isNotEmpty(inspLt)) {
			this.printMessage(request, response, String.format("错误原因：该%s已经录入了对应的行政执法检查工作记录，不能修改", "行政执法立项信息"),
			        ATTR_ERROR);
			return null;
		}
		RequestUtil.setFormFromBean(dyna, bs);
	 ...
	 * 行政执法信息修改.
	 */
	public ActionForward updateAdminEnforce(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			this.resetToken(request);// 清空当前会话中的token值
			// 如果当前用户不属于人行用户，则不能使用该功能
			...
			DynaActionForm dyna = (DynaActionForm) form;
			String nowLoginUser = this.getPubCredential(UserCredentialName.nickname.name(), request, response);
			String nowloginerOrgNo = this.getPubCredential(UserCredentialName.organization.name(), request, response);
			...
			try {
				...
				ae.setAebasis(StringUtil.join(aebasisArr, StringUtils.EMPTY, StringUtils.EMPTY, ","));
				ae.setAeway(StringUtil.join(aewayArr, StringUtils.EMPTY, StringUtils.EMPTY, ","));
				ae.setAeplanstdate(DateUtils.parseDate(dyna.getString("aeplanstdate"), DateUtil.DATE_FORMAT_ARRAY));
				ae.setAeplantmdate(DateUtils.parseDate(dyna.getString("aeplantmdate"), DateUtil.DATE_FORMAT_ARRAY));
				...
				ae.setDeptauditdate(DateUtils.parseDate(dyna.getString("deptauditdate"), DateUtil.DATE_FORMAT_ARRAY));
				ae.setChairopnion(dyna.getString("chairopnion"));
				ae.setChairman(dyna.getString("chairman"));
				ae.setAelimt(dyna.getString("aelimt"));
				ae.setChairauditdate(DateUtils.parseDate(dyna.getString("chairauditdate"), DateUtil.DATE_FORMAT_ARRAY));
				...
				LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改行政执法信息", "操作成功!"));
			} catch (Exception e) {
				LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改行政执法信息", "操作失败!"), e);
	 ...
	 * 行政执法信息删除.
	 */
	public ActionForward delAdminEnforce(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		try {
			...
			LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "删除行政执法登记", "操作成功!"));
		} catch (Exception e) {
			LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "删除行政执法登记", "操作失败!"));
	 ...
	 * 查看立项检查表.
	 */
	public ActionForward toSeeCheckPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// DynaActionForm dyna = (DynaActionForm) form;
		// String id = request.getParameter("id");
		// 调用检查方案下载方法
	 ...
	 * 跳转检查工作记录录入页面.
	 */
	public ActionForward toAeinspectionInitPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 产生令牌值
		...
		/*
		 * 工作检查记录编号隐藏标示，用于判断当前提交记录编号是否重复 用于进场、离场、事实认定书的重复判断
		 */
		...
		// 获取立项信息中维护的检查记录当前序号
		dyna.set("maxino", (af != null && af.getMaxino() != null) ? String.valueOf(af.getMaxino()) : "1");
	 ...
	 * 保存工作检查记录.
	 */
	public ActionForward createAeinspection(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			this.resetToken(request);// 清空当前会话中的token值
			// 如果当前用户不属于人行用户，则不能使用该功能
			...
			try {
				...
				// 从页面获取检查组组长
				bs.setAeheadman(request.getParameter("aeHeadmanToServer").replace(" ", StringUtils.EMPTY));
				// 从页面获取检查组主查人
				...
				String ino = dyna.get("aeno").toString() + "第【" + dyna.get("maxino").toString() + "】号";
				bs.setIno(ino);
				/*
				 * 工作检查记录在保存内部编号时，需要判断当前立项信息的行内编号是否为空 如果为空，则不保存工作记录的行内编号。
				 */
				if (StringUtils.isBlank(dyna.get("innerno").toString())) {
					bs.setInnerno(StringUtils.EMPTY);
				} else {
					String innerno = dyna.get("innerno").toString() + "第【" + dyna.get("maxino").toString() + "】号";
				...
				try {
					...
					try {
						/*
						 * 则删除当前该工作记录相关的其他信息 例如进场、离场、工作底稿、事实认定书、会谈纪要、执法情况统计等
						 */
					...
					// 并提示用户
					request.setAttribute(ATTR_MESSAGE,
					        "<b>保存行政执法登记操作失败,编号重复</b><br><b>失败原因:</b>因为在你保存执法工作检查记录之前有其他用户已经保存了工作检查编号 " + bs.getIno()
					                + " 的工作检查信息<br><b>解决方法:</b>请重新点击'检查工作记录登记'菜单获取新工作检查编号，再输入保存!");
					return mapping.findForward("toAdminEnforceMessage");
				}
				request.setAttribute(ATTR_MESSAGE, "保存行政执法工作检查记录【" + bs.getIno() + "】操作成功!");
				LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "保存检查工作记录", "操作成功!"));
			} catch (Exception e) {
				LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "保存检查工作记录", "操作失败!"), e);
	 ...
	 * @return
	 */
	public ActionForward isInputAllWork(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward isExsitAeInspc(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward saveBasis(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String nowLoginUserName = this.getPubCredential(UserCredentialName.nickname.name(), request, response);
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward saveBasisAtUpdateInspc(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String nowLoginUserName = this.getPubCredential(UserCredentialName.nickname.name(), request, response);
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
	 ...
	 * @return
	 */
	public ActionForward queryBasises(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward pageBasises(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward getBasisContent(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
		...
		// newWb.setFiled5(StringUtils.rightPad(wb.getFiled5(), 100,
		// "如需查看更多详情，请导出"));
	 ...
	 * @return
	 */
	public ActionForward deleteBasis(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward saveComing(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String nowLoginUserName = this.getPubCredential(UserCredentialName.nickname.name(), request, response);
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		...
		if (StringUtils.isNotBlank(request.getParameter("field7"))) {
			/*
			 * 目前录入工作检查记录者只会同一时间录入一笔工作检查记录，制度上将不允许并发操作 ，因此一旦存在进场记录，编号就维持不变
			 */
			...
			if (wc == null) {
				...
				if (bs == null) {
					...
					wc.setFiled10(noGenerator.generateNoStr(bs.getComeinnotext(), bs.getComeinnoyear(),
					        String.valueOf(bs.getComeinindex())));
				}
				isNew = true;
			} else {
	 ...
	 * @return
	 */
	public ActionForward pageComings(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward getComingContent(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward saveGoaway(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String nowLoginUserName = this.getPubCredential(UserCredentialName.nickname.name(), request, response);
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		...
		if (StringUtils.isNotBlank(request.getParameter("field14"))) {
			// wb =
			// enforceService.getWorkgoaway(request.getParameter("field14"),
			// request.getParameter("inspect_token"));
			...
			if (wb == null) {
				...
				if (bs == null) {
					...
					String filed10 = noGenerator.generateNoStr(bs.getAwaynotext(), bs.getAwaynoyear(),
					        String.valueOf(bs.getAwaynoindex()));
	 ...
	 * @return
	 */
	public ActionForward pageGoaways(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward getGoawayContent(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward saveTalkSummary(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String nowLoginUserName = this.getPubCredential(UserCredentialName.nickname.name(), request, response);
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		...
		if (StringUtils.isNotBlank(request.getParameter("field16"))) {
			// ws =
			// enforceService.getWorktalksummary(request.getParameter("field16"),
			// request.getParameter("inspect_token"));
	 ...
	 * @return
	 */
	public ActionForward pageTalkSummarys(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward getTalkSummaryContent(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward getProblemSummay(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
	 ...
	 * @return
	 */
	public ActionForward generateFactBook(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
	 ...
	 * @return
	 */
	public ActionForward saveFactBook(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String aeNo = request.getParameter("aeno");
		...
		try {
			factBookNo = enforceService.saveFactBook(aeNo, inspectionNo, bsorg.getName(), nowloginerOrgNo, aeedOrg,
			        factBookContent, filed6, filed12, filed9, selectedwtgk, aeorgno, aeorgnm, aeedorgno, aeedorgnm,
			        inspectToken);
		} catch (Exception e) {
			LOGGER.error("保存事实认定书内容发生错误", e);
			if (out != null) {
	 ...
	 * @return
	 */
	public ActionForward pageFactBook(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward saveInspectionAnl(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String nowLoginUserName = this.getPubCredential(UserCredentialName.nickname.name(), request, response);
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		...
		if (StringUtils.isNotBlank(request.getParameter("aeinspectionno"))) {
			// wb =
			// enforceService.getInspectionAnl(request.getParameter("aeinspectionno"),
			// request.getParameter("inspect_token"));
	 ...
	 * @return
	 */
	public ActionForward pageInspectionAnl(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward generateWord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		...
		if (StringUtils.equals(from, "detail")) {
			...
			aeinspectionNo = request.getParameter("aeno") + "第【" + request.getParameter("maxino") + "】号";
		}
		String inspectToken = request.getParameter("inspect_token");
		BsFactbook fb = null;
		try {
		...
		if (fb == null) {
			super.printMessage(request, response, "还未添加事实认定书" + aeinspectionNo + "，请添加后再导出！", ATTR_ERROR);
			return null;
		}
		 ...
		 * 表格外的参数param3使用“\r”换行，而表格外的参数param4使用“(char)11”换行 去除字符串中的空格,回车,换行符,制表符
		 */
		String f5 = fb.getFiled5().replaceAll("<BR>", "\r    ").replaceAll("<br>", "\r    ")
		        .replaceAll("</?[^>]+>", StringUtils.EMPTY).replaceAll("<a>\\s*|\t|\r|\n</a>", StringUtils.EMPTY)
		        .replace("&nbsp;", " ");
		...
		try {
			...
			response.addHeader("Content-Disposition",
			        "attachment;filename=" + super.getDownloadFileName(fb.getFiled7(), StringUtils.EMPTY));
			repos = response.getOutputStream();
			...
			LOGGER.error("生成事实认定书Word文件发生错误", e);
			super.printMessage(request, response, "事实认定书" + aeinspectionNo + "导出失败！错误原因:" + e.getMessage(), ATTR_ERROR);
			return null;
		} finally {
			try {
				if (fis != null) {
	 ...
	 * @return
	 */
	public ActionForward generateGzdgWord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String wordTemplateId = request.getParameter("wordTemplateId").toString();
		...
		try {
			...
			if (fb == null) {
				super.printMessage(request, response, "还未添加工作底稿" + aeinspectionNo + "，请添加后再导出！", ATTR_ERROR);
				return null;
			}
			String nowloginerOrgNo = this.getPubCredential(UserCredentialName.organization.name(), request, response);
			BsOrg bsorg = systemBaseInfoManager.getOrgByNo(nowloginerOrgNo);
			...
			String htmlContent = fb.getFiled5().replaceAll("<BR>", "\r    ").replaceAll("<br>", "\r    ")
			        .replaceAll("</?[^>]+>", StringUtils.EMPTY).replaceAll("<a>\\s*|\t|\r|\n</a>", StringUtils.EMPTY)
			        .replace("&nbsp;", " ");
			...
			super.printMessage(request, response, "工作底稿" + aeinspectionNo + "导出失败！错误原因:" + e.getMessage(), ATTR_ERROR);
			return null;
		} finally {
			try {
				if (fis != null) {
	 ...
	 * @return
	 */
	public ActionForward generateJcjlWord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String wordTemplateId = request.getParameter("wordTemplateId").toString();
		String aeinspectionNo = request.getParameter("ino");
		String inspectToken = request.getParameter("inspect_token");
		...
		try {
			...
			if (fb == null) {
				super.printMessage(request, response, "还未添加进场会谈记录" + aeinspectionNo + "，请添加后再导出！", ATTR_ERROR);
				return null;
			}
			String nowloginerOrgNo = this.getPubCredential(UserCredentialName.organization.name(), request, response);
			BsOrg bsorg = systemBaseInfoManager.getOrgByNo(nowloginerOrgNo);
			...
			response.setContentType("application/vnd.ms-word");
			response.addHeader("Content-Disposition",
			        "attachment;filename=" + super.getDownloadFileName(fb.getFiled10(), StringUtils.EMPTY));
			repos = response.getOutputStream();
			wordGenerator.replaceDoc(fis, map, repos);
			repos.flush();
			return null;
			...
			super.printMessage(request, response, "进场会谈记录" + aeinspectionNo + "导出失败！错误原因:" + e.getMessage(), ATTR_ERROR);
			return null;
		} finally {
			try {
				if (fis != null) {
	 ...
	 * @return
	 */
	public ActionForward generateLchtjlWord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String wordTemplateId = request.getParameter("wordTemplateId").toString();
		String aeinspectionNo = request.getParameter("ino");
		String inspectToken = request.getParameter("inspect_token");
		...
		try {
			...
			if (fb == null) {
				super.printMessage(request, response, "还未添加离场会谈纪要" + aeinspectionNo + "，请添加后再导出！", ATTR_ERROR);
				return null;
			}
			String nowloginerOrgNo = this.getPubCredential(UserCredentialName.organization.name(), request, response);
			BsOrg bsorg = systemBaseInfoManager.getOrgByNo(nowloginerOrgNo);
			...
			map.put("FILED6", fb.getFiled6());
			map.put("FILED7", fb.getFiled7());
			String htmlContent = fb.getFiled8().replaceAll("<BR>", "\r    ").replaceAll("<br>", "\r    ")
			        .replaceAll("</?[^>]+>", StringUtils.EMPTY).replaceAll("<a>\\s*|\t|\r|\n</a>", StringUtils.EMPTY)
			        .replace("&nbsp;", " ");
			...
			super.printMessage(request, response, "离场会谈纪要" + aeinspectionNo + "导出失败！错误原因:" + e.getMessage(), ATTR_ERROR);
			return null;
		} finally {
			try {
				if (fis != null) {
	 ...
	 * @return
	 */
	public ActionForward generateHtjyWord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String wordTemplateId = request.getParameter("wordTemplateId").toString();
		String aeinspectionNo = request.getParameter("ino");
		String inspectToken = request.getParameter("inspect_token");
		...
		try {
			...
			if (fb == null) {
				super.printMessage(request, response, "还未添加会谈纪要" + aeinspectionNo + "，请添加后再导出！", ATTR_ERROR);
				return null;
			}
			String nowloginerOrgNo = this.getPubCredential(UserCredentialName.organization.name(), request, response);
			BsOrg bsorg = systemBaseInfoManager.getOrgByNo(nowloginerOrgNo);
			...
			map.put("FILED5", fb.getFiled5());
			map.put("FILED6", fb.getFiled6());
			String htmlContent = fb.getFiled10().replaceAll("<BR>", "\r    ").replaceAll("<br>", "\r    ")
			        .replaceAll("</?[^>]+>", StringUtils.EMPTY).replaceAll("<a>\\s*|\t|\r|\n</a>", StringUtils.EMPTY)
			        .replace("&nbsp;", " ");
			...
			super.printMessage(request, response, "会谈纪要" + aeinspectionNo + "导出失败！错误原因:" + e.getMessage(), ATTR_ERROR);
			return null;
		} finally {
			try {
				if (fis != null) {
	 ...
	 * @return
	 */
	public ActionForward generateLxspbWord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String wordTemplateId = request.getParameter("wordTemplateId").toString();
		String aeNo = request.getParameter("aeno");
		FileInputStream fis = null;
		...
		try {
			...
			super.printMessage(request, response, "检查立项表" + aeNo + "导出失败！错误原因:" + e.getMessage(), ATTR_ERROR);
			return null;
		} finally {
			try {
				if (fis != null) {
	...
	public ActionForward toAeinspectionList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		String pageSize = (dyna.get("pageSize") == null || dyna.get("pageSize").equals(StringUtils.EMPTY)) ? PAGE_CONTAIN_NUMBER
		        .toString() : (String) dyna.get("pageSize");
		String ino = (dyna.get("ino") == null || dyna.get("ino").equals(StringUtils.EMPTY)) ? null : (String) dyna
		        .get("ino");
		String aeorgChoice = (dyna.get("aeorgChoice") == null || dyna.get("aeorgChoice").equals(StringUtils.EMPTY)) ? null
		        : (String) dyna.get("aeorgChoice");
		String aeedorgChoice = (dyna.get("aeedorgChoice") == null || dyna.get("aeedorgChoice")
		...
		Page page = enforceService.queryAeinspection(ino, aeorgChoice, aeedorgChoice, aeplanstdate, aeplantmdate,
		        Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		pageCountTemp = Long.valueOf(page.getTotalCount()).intValue();
		if (pageCountTemp == 0) {
			pageCountTemp = 1;
	 ...
	 * 点击登记执法检查结论菜单.
	 */
	public ActionForward toAeinspListForCreateConcl(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		Page page = enforceService.queryAeinspectionForCon(ino, aeorgChoice, aeedorgChoice, aeplanstdate, aeplantmdate,
		        Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		pageCountTemp = Long.valueOf(page.getTotalCount()).intValue();
		if (pageCountTemp == 0) {
			pageCountTemp = 1;
	 ...
	 * @return
	 */
	public ActionForward pageAeInspectionAnalyse(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
		...
		Page page = null;
		try {
			page = enforceService.pageAeInspectionAnalyse(a1, b1, stDate, tmDate, pageNo, limit, nowLoginUser);
		} catch (Exception e) {
			LOGGER.error("分页查询执法检查情况统计表发生错误", e);
			out.print("{totalProperty:0,\"root\":[]}");
			return null;
	 ...
	 * 跳转检查工作记录详细页面.
	 */
	public ActionForward toAeinspectionDetailPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
	 ...
	 * 跳转检查工作记录修改页面.
	 */
	public ActionForward toUpdateAeinspectionPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		this.saveToken(request);// 产生令牌值
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
		...
		// 判断当前用户是否有权限修改该信息
		if (bs == null) {
			this.printMessage(request, response, String.format(ERROR8, id, "行政执法工作检查记录"), ATTR_ERROR);
			return null;
		} else {
			String crtLogin = bs.getCrtlogin();
			String nowLoginUser = this.getPrivCredential(UserCredentialName.login.name(), request, response);
			if (StringUtils.isBlank(crtLogin)) {
				this.printMessage(request, response, String.format(ERROR5, "行政执法工作检查记录"), ATTR_ERROR);
				return null;
			}
			if (StringUtils.isBlank(nowLoginUser)) {
				this.printMessage(request, response, ERROR6, ATTR_ERROR);
			...
			if (!nowLoginUser.trim().equals(crtLogin.trim())) {
				this.printMessage(request, response, String.format(ERROR7, "行政执法工作检查记录"), ATTR_ERROR);
				return null;
			}
		}
	 ...
	 * 检查工作记录修改.
	 */
	public ActionForward updateAeinspection(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			this.resetToken(request);// 清空当前会话中的token值
			// 如果当前用户不属于人行用户，则不能使用该功能
			...
			try {
				...
				bs.setAeplanstdate(DateUtils.parseDate(dyna.getString("aeplanstdate"), DateUtil.DATE_FORMAT_ARRAY));
				bs.setAeplantmdate(DateUtils.parseDate(dyna.getString("aeplantmdate"), DateUtil.DATE_FORMAT_ARRAY));
				...
				bs.setCrtdate(new Date());
				bs.setCreator(this.getPrivCredential(UserCredentialName.login.name(), request, response));
				...
				LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改行政执法工作检查记录", "操作成功!"));
			} catch (Exception e) {
				LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改行政执法工作检查记录", "操作失败!"), e);
	 ...
	 * 检查工作记录删除.
	 */
	public ActionForward delAeinspection(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		try {
			...
			LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "删除检查工作记录", "操作成功!"));
		} catch (Exception e) {
			LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "删除检查工作记录", "操作失败!"), e);
	 ...
	 * 跳转检查结论登记页面.
	 */
	public ActionForward toAeconclusionInitPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		this.saveToken(request);// 产生令牌值
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
		...
		if (enforceService.getAeconclusionByAeno(aeinsp.getIno()) != null) {
			request.setAttribute(ATTR_MESSAGE, "<b>保存行政执法检查结论失败</b><br><b>失败原因:</b>已经存在执法检查工作记录编号为【" + aeinsp.getIno()
		...
		dyna.set("aeplanstdate", DateFormatUtils.format(aeinsp.getAeplanstdate(), DateUtil.DATE_FORMAT_YYYY_MM_DD));
		dyna.set("aeplantmdate", DateFormatUtils.format(aeinsp.getAeplantmdate(), DateUtil.DATE_FORMAT_YYYY_MM_DD));
	 ...
	 * @return
	 */
	public ActionForward getUsersByOrgNo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
		...
		for (int i = 0; i < userlt.size(); i++) {
			...
			outputSb.append("{'value':'").append(bsUserInfoOfJg.getName()).append("--")
			        .append(bsUserInfoOfJg.getLoginId()).append("','text':'").append(bsUserInfoOfJg.getName())
			        .append("-").append(bsUserInfoOfJg.getLoginId()).append("'}");
			if (i != userlt.size() - 1) {
				outputSb.append(",");
			}
		}
	 ...
	 * @return
	 */
	public ActionForward getUsersByOrgNos(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward getOrgsByParentOrgNo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
		...
		StringBuffer outputSb = new StringBuffer("{totalProperty:" + childrenOrgslt.size() + ",root:[");
		BsOrg org = null;
		for (int i = 0; i < childrenOrgslt.size(); i++) {
			org = childrenOrgslt.get(i);
			outputSb.append("{'value':'").append(org.getName()).append("--").append(org.getNo()).append("','text':'")
			        .append(org.getName()).append("-").append(org.getNo()).append("'}");
	 ...
	 * 检查结论保存方法.
	 */
	public ActionForward createAeconclusion(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			this.resetToken(request);// 清空当前会话中的token值
			...
			try {
				...
				try {
					...
					request.setAttribute(ATTR_MESSAGE, "<b>保存检查结论操作失败,执法检查意见书编号重复</b><br><b>失败原因:</b>因为在你填写的执法检查意见书编号 "
				...
				LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "保存检查结论", "操作失败!"), e);
			}
			return mapping.findForward("toAdminEnforceMessage");
		} else {// 重复提交
	 ...
	 * 跳转检查结论列表页面.
	 */
	public ActionForward toAeconclusionList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		String pageSize = (dyna.get("pageSize") == null || dyna.get("pageSize").equals(StringUtils.EMPTY)) ? PAGE_CONTAIN_NUMBER
		        .toString() : (String) dyna.get("pageSize");
		String aeno = (dyna.get("aeopnionno") == null || dyna.get("aeopnionno").equals(StringUtils.EMPTY)) ? null
		        : (String) dyna.get("aeopnionno");
		String aeorgChoice = (dyna.get("aeorgChoice") == null || dyna.get("aeorgChoice").equals(StringUtils.EMPTY)) ? null
		        : (String) dyna.get("aeorgChoice");
		String aeedorgChoice = (dyna.get("aeedorgChoice") == null || dyna.get("aeedorgChoice")
		        .equals(StringUtils.EMPTY)) ? null : (String) dyna.get("aeedorgChoice");
		...
		if (source != null && source.trim().equals("punish")) {
			page = enforceService.queryAeconclusionWithPunish(aeno, aeorgChoice, aeedorgChoice, aeplanstdate,
			        aeplantmdate, Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		} else {
			page = enforceService.queryAeconclusion(aeno, aeorgChoice, aeedorgChoice, aeplanstdate, aeplantmdate,
			        Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		}
		pageCountTemp = Long.valueOf(page.getTotalCount()).intValue();
		if (pageCountTemp == 0) {
	 ...
	 * 跳转检查结论详细页面.
	 */
	public ActionForward toAeconclusionDetailPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
	 ...
	 * 跳转检查结论修改页面.
	 */
	public ActionForward toUpdateAeconclusionPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		this.saveToken(request);// 产生令牌值
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
		...
		if (BooleanUtils.isFalse(bs.isCanbeupdate())) {
			this.printMessage(request, response, String.format(ERROR9, bs.getAeno(), "执法检查结论"), ATTR_ERROR);
			return null;
		}
		...
		if (CollectionUtils.isNotEmpty(pnshLt)) {
			this.printMessage(request, response, String.format("错误原因：该%s已经录入了对应的处罚立项信息，不能修改", "检查结论信息"), ATTR_ERROR);
			return null;
		}

		RequestUtil.setFormFromBean(dyna, bs);
		String relorgnm = (dyna.get("relorgnm") == null || dyna.get("relorgnm").equals(StringUtils.EMPTY)) ? null
		        : (String) dyna.get("relorgnm");
	 ...
	 * 检查结论修改.
	 */
	public ActionForward updateAeconclusion(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			this.resetToken(request);// 清空当前会话中的token值
			// 如果当前用户不属于人行用户，则不能使用该功能
			...
			if (bs != null) {
				...
				if (CollectionUtils.isNotEmpty(punishs) && !bs.getDecision().trim().equals(decisionStr.trim())) {
			...
			try {
				...
				LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改检查结论", "操作成功！"));
			} catch (Exception e) {
				LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改检查结论", "操作失败!"), e);
	 ...
	 * 检查结论删除.
	 */
	public ActionForward delAeconclusion(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		try {
			...
			LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "删除检查结论", "操作失败!"), e);
	 ...
	 * 点击登记执法检查整改记录菜单.
	 */
	public ActionForward toAeinspListForCreateRecti(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		Page page = enforceService.queryAeinspectionForRefti(ino, aeorgChoice, aeedorgChoice, aeplanstdate,
		        aeplantmdate, Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		pageCountTemp = Long.valueOf(page.getTotalCount()).intValue();
		if (pageCountTemp == 0) {
			pageCountTemp = 1;
	 ...
	 * 跳转跟踪整改信息录入页面.
	 */
	public ActionForward toAerectificationInitPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		this.saveToken(request);// 产生令牌值
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
		...
		if (enforceService.getAerectificationByIno(aeinsp.getIno()) != null) {
			request.setAttribute(ATTR_MESSAGE, "<b>保存行政执法整改信息失败</b><br><b>失败原因:</b>已经存在执法检查工作记录编号为【" + aeinsp.getIno()
		...
		if (bs == null) {
			request.setAttribute("methodname", "toAeinspListForCreateRecti");
			request.setAttribute(ATTR_MESSAGE, "<b>跳转录入整改页面失败,当前执法信息还未录入工作检查结论</b><br><b>失败原因:</b>当前还未录入工作检查记录 "
			        + aeinsp.getIno() + " 的检查结论信息，请录入后再录入整改信息<br><b>解决方法:</b>请重新点击'检查结论登记'菜单选择工作记录录入结论信息!");
			return mapping.findForward("toAdminEnforceMessage");
		}
		dyna.set("aeopnionno", bs.getAeopnionno());
		dyna.set("trackno", aeno + DateFormatUtils.format(new Date(), DateUtil.DATE_FORMAT_YYYYMMDDHHMMSSSSS));
	 ...
	 * 跟踪整改信息保存方法.
	 */
	public ActionForward createAerectification(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			this.resetToken(request);// 清空当前会话中的token值
			// 如果当前用户不属于人行用户，则不能使用该功能
			...
			// 调用跟踪整改信息保存方法
			try {
				BsAerectification bs = RequestUtil.getBeanFromParams(request, BsAerectification.class);
				...
				try {
					...
					request.setAttribute(ATTR_MESSAGE,
					        "<b>保存整改信息操作失败,跟踪整改编号重复</b><br><b>失败原因:</b>因为在你保存执法信息之前有其他用户已经保存了跟踪整改编号 " + bs.getTrackno()
				...
				LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "保存跟踪整改信息", "操作成功！"));
			} catch (Exception e) {
				LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "保存跟踪整改信息", "操作失败!"), e);
	 ...
	 * 跳转跟踪整改信息列表页面.
	 */
	public ActionForward toAerectificationList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		String pageSize = (dyna.get("pageSize") == null || dyna.get("pageSize").equals(StringUtils.EMPTY)) ? PAGE_CONTAIN_NUMBER
		        .toString() : (String) dyna.get("pageSize");
		String trackno = (dyna.get("trackno") == null || dyna.get("trackno").equals(StringUtils.EMPTY)) ? null
		        : (String) dyna.get("trackno");
		String aeorgChoice = (dyna.get("aeorgChoice") == null || dyna.get("aeorgChoice").equals(StringUtils.EMPTY)) ? null
		        : (String) dyna.get("aeorgChoice");
		String aeedorgChoice = (dyna.get("aeedorgChoice") == null || dyna.get("aeedorgChoice")
		        .equals(StringUtils.EMPTY)) ? null : (String) dyna.get("aeedorgChoice");
		...
		Page page = enforceService.queryAerectification(trackno, aeorgChoice, aeedorgChoice, aeplanstdate,
		        aeplantmdate, Integer.parseInt(pageNo), Integer.parseInt(pageSize), loginOrgNo);
		pageCountTemp = Long.valueOf(page.getTotalCount()).intValue();
		if (pageCountTemp == 0) {
			pageCountTemp = 1;
	...
	public ActionForward toUpdateAerectificationPage(ActionMapping mapping, ActionForm form,
		...
		if (BooleanUtils.isFalse(bs.isCanbeupdate())) {
			this.printMessage(request, response, String.format(ERROR9, bs.getAeno(), "跟踪整改信息"), ATTR_ERROR);
			return null;
		}
		RequestUtil.setFormFromBean(dyna, bs);
		// 由于数据库保存内容中换行符已转义，现需要饭转义
	 ...
	 * 跟踪整改信息修改.
	 */
	public ActionForward updateAerectification(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		if (this.isTokenValid(request)) {// 正常提交
			this.resetToken(request);// 清空当前会话中的token值
			// 如果当前用户不属于人行用户，则不能使用该功能
			...
			try {
				...
				if (dyna.get("rectificationstdate") != null
				        && !dyna.get("rectificationstdate").toString().trim().equals(StringUtils.EMPTY)) {
					bs.setRectificationstdate(DateUtils.parseDate(dyna.get("rectificationstdate").toString(),
					        DateUtil.DATE_FORMAT_ARRAY));
				}
				if (dyna.get("rectificationtmdate") != null
				        && !dyna.get("rectificationtmdate").toString().trim().equals(StringUtils.EMPTY)) {
					bs.setRectificationtmdate(DateUtils.parseDate(dyna.get("rectificationtmdate").toString(),
					        DateUtil.DATE_FORMAT_ARRAY));
				}
				bs.setUpdateate(new Date());
				enforceService.saveAerectification(bs);
				request.setAttribute(ATTR_MESSAGE, "修改行政执法整改情况【" + bs.getAeno() + "】操作成功！");
				LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改跟踪整改信息", "操作成功!"));
			} catch (Exception e) {
				LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "修改跟踪整改信息", "操作失败!"), e);
	 ...
	 * 跟踪整改信息删除.
	 */
	public ActionForward delAerectification(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
		...
		try {
			...
			LOGGER.info(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "删除跟踪整改信息", "操作成功!"));
		} catch (Exception e) {
			LOGGER.error(String.format(LOG_FORMAT, bsorg.getName(), nowLoginUser, "删除跟踪整改信息", "操作失败!"), e);
	 ...
	 * @return
	 */
	public ActionForward getDictionary(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ActionForward getLoginInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		response.setContentType("text/html;charset=UTF-8");
		response.setHeader("Content-type", "text/html;charset=UTF-8");
		response.setCharacterEncoding("UTF-8");
		...
		try {
			...
			String orgNo = super.getPubCredential(UserCredentialName.organization.name(), request, response);
			...
			String login = super.getPrivCredential(UserCredentialName.login.name(), request, response);
			if (StringUtils.isNotBlank(login)) {
				// 屏蔽身份证号，eg：
				if (login.length() >= 15) {
					outputStr.append(StringUtils.overlay(StringUtils.overlay(login, "*****", 1, 6), "***", 14, 17));
				} else {
					outputStr.append(login);
				}
			}
			outputStr.append("',nickname:'");
			String nickname = super.getPubCredential(UserCredentialName.nickname.name(), request, response);
	 ...
	 * @return
	 */
	public ActionForward getAeedOrg(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
	 ...
	 * @param response
	 */
	public void selectChildInvolveBank(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		String parent = request.getParameter("node");
		/*
		 * String nowloginerOrgNo = this.getPubCredential(
	 ...
	 * @return
	 */
	public ActionForward exportCvs(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// 如果当前用户不属于人行用户，则不能使用该功能
		if (!super.isPcbUser(request, response)) {
			this.printMessage(request, response, ERROR1, ATTR_ERROR);
	...
	public ActionForward exportXls(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		case 9:
			fileName = "行政处罚立项";
			wb = admpunishStsicSvce.generateExcel(request.getParameter("obj"), admpunishStsicSvce.doStatistics(params),
			        keyValue);
			break;
		case 10:
			fileName = "行政处罚结论";
	 ...
	 * @return
	 */
	public ActionForward generateAeFeedBackExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	        HttpServletResponse response) {
		// DynaActionForm dyna = (DynaActionForm) form;
		String nowLoginUser = this.getPubCredential(UserCredentialName.organization.name(), request, response);
		if (StringUtils.isBlank(nowLoginUser)) {
		...
		try {
			repos = response.getOutputStream();
			HSSFWorkbook wb = null;
			Map<String, Object> data = aeFeedbackStsicSvce.doStatistics(params);
			wb = aeFeedbackStsicSvce.generateExcel("4", data, keyValue);
			String fileName = String.format("意见反馈信息表-%1$tm%1$td%1$tH%1$tM%1$tS.xls", new Date());
			response.setContentType("application/vnd.ms-excel");
			response.addHeader("Content-Disposition", "attachment;filename="
			        + new String(fileName.getBytes("gb2312"), "ISO8859-1"));
			// FileOutputStream fos=new FileOutputStream("F:/aaaa.xls");
			...
			LOGGER.error("生成意见反馈信息表Excel文件发生错误", e);
		} finally {
			IOUtils.closeQuietly(repos);
		}
		return null;
