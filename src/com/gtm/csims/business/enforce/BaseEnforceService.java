...
public class BaseEnforceService {
    ...
    public List<BsUserInfoOfJg> getUsersByOrgNo(String orgNo) {
        List<BsUserInfoOfJg> list = bsUserInfoOfJgDao.find(
                "from BsUserInfoOfJg where OrgCodeOfJr = ?", orgNo);
        return list;
    }
    ...
    public List<BsUserInfoOfJg> getUsersByOrgNos(String orgNos) {
        List<BsUserInfoOfJg> list = bsUserInfoOfJgDao.find(
                "from BsUserInfoOfJg where OrgCodeOfJr in (" + orgNos + ")", new Object[] {});
        return list;
    }
    ...
    public BsUserInfoOfJg getUsersByUserId(String userId) {
        List<BsUserInfoOfJg> list = bsUserInfoOfJgDao.find("from BsUserInfoOfJg where loginid = '"
                + userId + "'", new Object[] {});
    ...
    public List<BsOrg> getOrgsByParentOrgNo(String parentNo) {
        List<BsOrg> list = bsOrgDao
                .find("from BsOrg where Parentno = ?", new Object[] { parentNo });
        return list;
    }
    ...
    public List<BsOrg> getOrgsByParentOrgNoContainSelf(String parentNo) {
        List<BsOrg> list = bsOrgDao.find(
                "from BsOrg where Parentno = ? or no = ? order by Parentno", new Object[] {
                        parentNo, parentNo });
        return list;
    }
    ...
    public List<BsOrg> getOrgsByParentOrgNoForZHOrXZH(String parentNo, String pbcNo) {
        ...
        List<BsOrg> list = bsOrgDao.find(
                "from BsOrg where Parentno = ? and " + this.getOrInClause("Pcbno in (%s)", child),
                parentNo);
        return list;
    }
        ...
        if (length > MAX_IN_CLUASE_NUMBER) {
            ...
            for (int i = 0; i < n; i++) {
                sb.append(String.format(cluase, StringUtil.join(
                        ArrayUtils.subarray(itemArr, i * MAX_IN_CLUASE_NUMBER, (i + 1)
                                * MAX_IN_CLUASE_NUMBER), StringUtils.EMPTY, StringUtils.EMPTY,
                        SEPARATOR)));
                if (i < n - 1) {
                    sb.append(" OR ");
                }
    ...
    public List<BsAepeople> getAepeoplesByPcbOrgNo(String pcbNo) {
        List<BsAepeople> list = bsAepeopleDao.find("from BsAepeople where bankno = ?",
                new Object[] { pcbNo });
        return list;
    }
    ...
    public String getWTGKContent(String[] selectedwtgk) {
        try {
            ...
            List<Map<String, Object>> parentlt = jdbcTemplate
                    .queryForList("SELECT distinct(stat) FROM BS_DICTIONARY WHERE id in ("
                            + inConditionStr + ")");
            ...
            List<BsDictionary> lt = bsDictionaryDao.find(
                    "from BsDictionary where  isenable = 'true' and id in (" + totalInConditionStr
    ...
    public void relatePersonProfile(String infoType, String personId, String personNm,
        ...
        try {
            List list = jdbcTemplate
                    .queryForList(
                            ...
                            new Object[] { login, UserCredentialName.login.name(),
                                    credential.name() }, java.lang.String.class);
