...
public class SystemBaseInfoManagerImpl implements SystemBaseInfoManager {
    ...
    public List<BsArea> areaList(String userAreaId, boolean isFirst) {
        try {
            ...
            if (isFirst == true) {
                areaHql.append(" where  ba.Id = '" + userAreaId
                        + "'  and ba.Status = '1' ");
            } else {
                areaHql.append(" where  ba.Parent.Id = '" + userAreaId
                        + "'  and ba.Status = '1' ");
            }
    ...
    public List<BsOrg> orgListByParentNo(String parentNo) {
        try {
            StringBuffer areaHql = new StringBuffer();
            areaHql.append(" from BsOrg where parentno = '" + parentNo
                    + "' order by no asc ");
    ...
    public Page getDeptList(String deptName, String orgNo, int pageNo,
            int pageSize) {
        ...
        Page page = bsDeptDao.pagedQuery(sb.toString(), pageNo, pageSize,
                param.toArray());
        return page;
    }

    public Page getOrgList(String orgNo, String orgName, String org,
        ...
        Page page = bsOrgDao.pagedQuery(sb.toString(), pageNo, pageSize,
                param.toArray());
        return page;
    }

    public String getOrgComInfoTR(String orgNo) {
        List orgComInfoList = bsOrgComInfoDao.find(
                "FROM BsOrgComInfo WHERE no = ?", orgNo);
        StringBuffer sb = new StringBuffer("");
        if (orgComInfoList.size() > 0) {
            sb.append(
                    "<tr class='tabletext02'><td align='center'>序号</td><td align='center'>表彰时间</td>")
                    .append("<td align='center'>表彰机构</td><td align='center'>表彰内容</td></tr>");
            for (int i = 0; i < orgComInfoList.size(); i++) {
                BsOrgComInfo bs = (BsOrgComInfo) orgComInfoList.get(i);
                sb.append("<tr><td align='center'>").append(i + 1)
                        .append("</td>");
                sb.append("<td align='center'>").append(bs.getComDate())
                        .append("</td>");
                sb.append("<td align='center'>").append(bs.getComOrgName())
                        .append("</td>");
                sb.append("<td align='center'>").append(bs.getComContent())
                        .append("</td></tr>");
            }
        }
        return sb.toString();
    ...
    public void savaOrgComInfo(InputStream is, String endRow) {
        ...
        try {
            ...
            for (int i = 1; i < Integer.parseInt(endRow); i++) {
                // 你把你的方法写道这
                String comedOrgNo = ExcelUtil.getValue(sheet.getRow(i).getCell(
                        0));
