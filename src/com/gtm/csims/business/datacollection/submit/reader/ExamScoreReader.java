	...
	protected void read(HSSFWorkbook wb, Map<String, String> reportParam) {
		...
		for (int row = super.config.getBeginRow(); row <= lastRowIndex - 1; row++) {
			try {
				...
				es.setExstdt(DateUtils.parseDate(ExcelUtil.getValue(sheet, row, 9).trim(), DateUtil.DATE_FORMAT_ARRAY));
				es.setExtmdt(DateUtils.parseDate(ExcelUtil.getValue(sheet, row, 10).trim(), DateUtil.DATE_FORMAT_ARRAY));
