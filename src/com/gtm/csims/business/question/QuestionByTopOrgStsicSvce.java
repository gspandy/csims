 ...
 * 
 */
public class QuestionByTopOrgStsicSvce extends BaseStatisticsService implements StatisticsService {

	private static Log LOGGER = LogFactory.getLog(QuestionByTopOrgStsicSvce.class);
	...
	private Map<String, Object> getResultData(String uuid, Map<String, String> paramsMap) {
		...
		// 根据字典表查询所有问题概况条款
		query_sql
		        .append("select (select qqtitle from BS_QUESTION where id = BSQUESTION)  AS qqtitle , ANSWERRESULT, Max(artoporgtypename) as orgType, ")
		        .append("dec(")
		        ...
		        .append("' group by BSQUESTION, ANSWERRESULT, artoporgtypeno order by BSQUESTION, ANSWERRESULT");
		...
		if (!CollectionUtils.isEmpty(results)) {
			...
			for (int i = 0; i < results.size(); i++) {
				...
				resultMap.put((i + 1) + "-3", eachMap.get("orgType"));
