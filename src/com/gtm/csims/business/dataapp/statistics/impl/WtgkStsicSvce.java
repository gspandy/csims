...
public class WtgkStsicSvce extends BaseStatisticsService implements StatisticsService {
    ...
    private Map<String, Object> getResultData(String uuid, Map<String, String> paramsMap) {
        ...
        queryWTGK_sql
                .append("SELECT id,dvalue FROM BS_DICTIONARY WHERE type = 'WTGK' ORDER BY dkey");
        ...
        if (wtgkDics != null && wtgkDics.size() > 0) {
            for (int i = 0; i < wtgkDics.size(); i++) {
                ...
                // 从第二行开始填充数据
                resultMap.put((i + 2) + "-1", wtgkDicsMap.get("id"));
        ...
        querySql.append("WITH s AS ")
                .append(" ( SELECT row_number() OVER(PARTITION BY AEEDORGNM ORDER BY id) id1,")
        ...
        if (paramsMap.get("FROM_DATE") != null
            ...
            querySql.append(" AND CREATEDATE >= '").append(paramsMap.get("FROM_DATE").toString())
                    .append("' ");
        if (paramsMap.get("TO_DATE") != null
                && !paramsMap.get("TO_DATE").toString().trim().equals(""))
            querySql.append(" AND  CREATEDATE <= '").append(paramsMap.get("TO_DATE").toString())
                    .append("' ");
        if (paramsMap.get("LOGIN_ORGNO") != null
            ...
            if (paramsMap.get("LOGIN_IS_PCB") != null
                ...
                if (!paramsMap.get("LOGIN_ORGNO").equals(Constants.PCB_SC_ORG_NO)) {
                    String childrenNoStr = super.getChildOrgByParentNoStr(paramsMap
                            .get("LOGIN_ORGNO"));
        ...
        querySql.append("   ) ,").append("   t ( AEEDORGNM,id1, id2,SELECTEDWTGK ) AS")
                .append("   (SELECT AEEDORGNM,id1,id2" + ",CAST(SELECTEDWTGK AS VARCHAR(2000))")
                .append("      FROM s").append("      WHERE id1 = 1").append("      AND id2 = 1")
                .append("      UNION ALL").append("        SELECT t.AEEDORGNM, t.id1 + 1, t.id2,")
                .append("           CAST(s.SELECTEDWTGK||''||t.SELECTEDWTGK AS VARCHAR(2000))")
                .append("            FROM s, t").append("            WHERE s.id2 = t.id1 + 1")
                .append("            AND t.AEEDORGNM =  s.AEEDORGNM )")
                .append("  SELECT AEEDORGNM,SELECTEDWTGK").append("  FROM t")
                .append("  WHERE t.id1 =").append("      (SELECT MAX(id1)")
