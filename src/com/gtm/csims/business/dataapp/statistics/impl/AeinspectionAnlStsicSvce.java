...
public class AeinspectionAnlStsicSvce extends BaseStatisticsService implements StatisticsService {
    ...
    private Map<String, Object> getResultData(String uuid, Map<String, String> paramsMap) {
        ...
        StringBuffer querySql = new StringBuffer(
                "SELECT A1,B1,C1,D1,E1,F1,G1,H1,I1,J1,K1,L1,M1,N1,P1 ").append(" FROM ")
                .append("BS_AEINSPECTION_ANL").append(" WHERE 1=1 ");
        if (paramsMap.get("FROM_DATE") != null
                && !paramsMap.get("FROM_DATE").toString().trim().equals(""))
            querySql.append(" AND ANLDATE >= '").append(paramsMap.get("FROM_DATE").toString())
                    .append("' ");
        if (paramsMap.get("TO_DATE") != null
                && !paramsMap.get("TO_DATE").toString().trim().equals(""))
            querySql.append(" AND  ANLDATE <= '").append(paramsMap.get("TO_DATE").toString())
                    .append("' ");
        ...
        if (paramsMap.get("ORGNO") != null && !paramsMap.get("ORGNO").toString().trim().equals("")
            ...
            String childrenNoStr = super.getChildOrgByParentNoStr(paramsMap.get("ORGNO").toString()
                    .trim());
        ...
        for (; j <= result.size(); j++) {// 返回数据条数
            ...
            for (i = 1; i <= this.getxCount(); i++) {// 17列(A1-Q1)
                try {
                    resultMap.put(j + "-" + i,
                            row.getValue(i - 1) == null ? "0" : row.getValue(i - 1).toString());
        ...
        StringBuffer querySql2 = new StringBuffer(
                ...
                .append(",SUM(decimal(L1,13,0)),SUM(decimal(M1,13,0)),SUM(decimal(P1,13,2))")
                .append(" FROM ").append("BS_AEINSPECTION_ANL WHERE 1=1 ");
        if (paramsMap.get("FROM_DATE") != null
                && !paramsMap.get("FROM_DATE").toString().trim().equals(""))
            querySql2.append(" AND ANLDATE >= '").append(paramsMap.get("FROM_DATE").toString())
                    .append("' ");
        if (paramsMap.get("TO_DATE") != null
                && !paramsMap.get("TO_DATE").toString().trim().equals(""))
            querySql2.append(" AND  ANLDATE <= '").append(paramsMap.get("TO_DATE").toString())
                    .append("' ");
        if (paramsMap.get("A1") != null && !paramsMap.get("A1").toString().trim().equals(""))
            querySql2.append(" AND A1 Like '%").append(paramsMap.get("A1").toString())
                    .append("%' ");
        if (paramsMap.get("B1") != null && !paramsMap.get("B1").toString().trim().equals(""))
            querySql2.append(" AND B1 Like '%").append(paramsMap.get("B1").toString())
                    .append("%' ");
