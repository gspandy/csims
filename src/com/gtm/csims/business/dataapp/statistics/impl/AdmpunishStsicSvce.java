 ...
 * 
 */
public class AdmpunishStsicSvce extends BaseStatisticsService implements StatisticsService {
    ...
    private Map<String, Object> getResultData(String uuid, Map<String, String> paramsMap) {
        ...
        List<?> result = this.enforceService.queryAllBsAdmpunish(paramsMap.get("ORGNO").toString());
        // System.out.println(result.size());
        BsAdmpunish row = null;
        for (int j = 1; j <= result.size(); j++) {// 返回数据条数
            // 循环每条数据
            try {
                row = (BsAdmpunish) result.get(j - 1);
            } catch (Exception e) {
                // 如果查询出错
            ...
            resultMap.put(j + "-" + 6, StringEscapeUtils.unescapeJava(row.getSummarys()));
            resultMap.put(j + "-" + 7, StringEscapeUtils.unescapeJava(row.getPeopleopnion()));
            resultMap.put(j + "-" + 8, row.getPeopler());
            resultMap.put(j + "-" + 9, DateFormatUtils.format(row.getPeoplerdate(), DATE_FORMAT));
            resultMap.put(j + "-" + 10, StringEscapeUtils.unescapeJava(row.getDptopnion()));
            resultMap.put(j + "-" + 11, row.getDpter());
            resultMap.put(j + "-" + 12, DateFormatUtils.format(row.getDpterdate(), DATE_FORMAT));
