 ...
 * 
 */
public class AeinspectionStsicSvce extends BaseStatisticsService implements StatisticsService {
	...
	private Map<String, Object> getResultData(String uuid, Map<String, String> paramsMap) {
		...
		List<?> result = this.enforceService.queryAllAeinspection(paramsMap.get("ORGNO").toString());
		// System.out.println(result.size());
		BsAeinspection row = null;
		for (int j = 1; j <= result.size(); j++) {// 返回数据条数
			// 循环每条数据
			try {
				row = (BsAeinspection) result.get(j - 1);
			} catch (Exception e) {
				// 如果查询出错
			...
			resultMap.put(j + "-" + 7, row.getAeheadman());
			resultMap.put(j + "-" + 8, row.getAemaster());
			resultMap.put(j + "-" + 9, row.getAeother());
