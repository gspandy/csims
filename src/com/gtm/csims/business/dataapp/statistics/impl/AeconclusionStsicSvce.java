 ...
 * 
 */
public class AeconclusionStsicSvce extends BaseStatisticsService implements StatisticsService {
    ...
    private Map<String, Object> getResultData(String uuid, Map<String, String> paramsMap) {
        ...
        List<?> result = this.enforceService.queryAllBsAeconclusion(paramsMap.get("ORGNO").toString());
        // System.out.println(result.size());
        BsAeconclusion row = null;
        for (int j = 1; j <= result.size(); j++) {// 返回数据条数
            // 循环每条数据
            try {
                row = (BsAeconclusion) result.get(j - 1);
            } catch (Exception e) {
                // 如果查询出错
